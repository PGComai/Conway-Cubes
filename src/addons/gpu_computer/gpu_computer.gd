...
var shader_textures := Dictionary()
var texture_rids := Dictionary()
...
func add_texture(
				_set: int,
				binding: int,
				_format: RenderingDevice.DataFormat,
				_texture_type: RenderingDevice.TextureType,
				size_whd: Vector3i,
				_array_layers: int,
				_mipmaps: int,
				_usage_bits: RenderingDevice.TextureUsageBits,
			) -> RID:
	var set_and_binding := Vector2i(_set, binding)
	var tex_format := RDTextureFormat.new()
	tex_format.format = _format
	tex_format.texture_type = _texture_type
	tex_format.width = size_whd.x
	tex_format.height = size_whd.y
	tex_format.depth = size_whd.z
	tex_format.array_layers = _array_layers
	tex_format.mipmaps = _mipmaps
	tex_format.usage_bits = _usage_bits
	shader_textures[set_and_binding] = rd.texture_create(tex_format,
														RDTextureView.new(),
														[])
	texture_rids[set_and_binding] = shader_textures[set_and_binding]
	return shader_textures[set_and_binding]
...
func _add_uniform(
				_uniform_type: RenderingDevice.UniformType,
				_set: int,
				_binding: int,
				id: RID
			) -> void:
...
func _make_pipeline(workgroup_size: Vector3i, new_pipeline: bool = false) -> void:
	if new_pipeline:
		...
		for set_and_binding in shader_textures.keys():
			_add_uniform(RenderingDevice.UNIFORM_TYPE_IMAGE,
					set_and_binding.x,
					set_and_binding.y,
					shader_textures[set_and_binding])
